...
import "../../system/defaultSearchBar.css";
import { BigBreadcrumbs, WidgetGrid, JarvisWidget } from "../../../smartadmin";
import axios from "axios";
import "../css/Code.css";
import Search from "../../../smartadmin/tableHeader/Search";
...
const AuthorizationMgmt = () => {
    ...
    //체크된 아이템의 AuthorizationMgmt 숫자만 저장
    const changeInt = selectedData.map((item) => item.AuthorizationMgmt);
            ...
            if (isChecked) {
                ...
                if (
                    !prevSelectedData.find(
                        (selectedItem) =>
                            selectedItem.AuthorizationMgmt === item.AuthorizationMgmt
                    )
                ) {
                    const sortedData = [...prevSelectedData, item].sort(
                        (a, b) => {
                            // AuthorizationMgmt 속성을 기준으로 데이터 정렬
                            if (a.AuthorizationMgmt < b.AuthorizationMgmt) {
                                return -1;
                            }
                            if (a.AuthorizationMgmt > b.AuthorizationMgmt) {
                                return 1;
                                                                            ...
                                                                            <input
                                                                                ...
                                                                                checked={selectedData.some(
                                                                                    (
                                                                                        ...
                                                                                        selectedItem.AuthorizationMgmt ===
                                                                                        item.AuthorizationMgmt
                                                                                )}
                                                                                onChange={(
                                                                                    e
                                                                                ) =>
