...
export default class JarvisWidget extends React.Component {
  ...
  render() {
    ...
    [
      ...
      if (!this.props[option]) widgetProps["data-widget-" + option] = false; //옵션 기본값
    });

    ["hidden", "collapsed"].forEach(option => {
      if (this.props[option]) widgetProps["data-widget-" + option] = true; //옵션 기본값
    });

    ["refresh", "load"].forEach(option => {
      if (this.props[option])
        widgetProps["data-widget-" + option] = this.props[option]; //옵션 기본값
    });
